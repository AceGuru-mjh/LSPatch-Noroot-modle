...
/**
 * Shizuku UI 自动�?截图/点击 Hook
 *
 * 通过 Shizuku adb 级执�?screencap + input tap 命令实现 UI 自动化，
 * 用于辅助视频下载等场景的按钮自动化点击�? *
 * 硬性限制：
 *  - 仅通过 Shizuku adb 级执�?screencap + input tap
 *  - 不调�?root 命令
 *  - 截图保存�?/sdcard/Download/
 */
object ShizukuCaptureHook {
    ...
    private fun performCapture(cfg: VideoConfig) {
        if (!ShizukuHelper.isAvailable()) {
            LogX.w("Shizuku不可用，跳过 screencap / input tap 自动�?)
            return
        }
