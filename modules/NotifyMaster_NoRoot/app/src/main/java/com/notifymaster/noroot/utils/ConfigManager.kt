...
object ConfigManager {
    ...
    const val PREFS_NAME = "notify_master_prefs"
    ...
    fun getGlobalConfig(): NotifyConfig {
        val def = NotifyConfig(packageName = "global")
        if (!isInitialized()) return def
        val json = prefs?.getString(KEY_GLOBAL, null) ?: return def
        return try { gson.fromJson(json, NotifyConfig::class.java) ?: def } catch (_: Throwable) { def }
    }

    fun saveGlobalConfig(cfg: NotifyConfig) {
    ...
    // ===== 单APP配置（保留接口，兼容旧逻辑�?=====
    fun getAllConfigs(): MutableMap<String, NotifyConfig> {
        if (!isInitialized()) return mutableMapOf()
        val json = prefs?.getString(KEY_ALL, null) ?: return mutableMapOf()
        return try {
            val type = object : TypeToken<MutableMap<String, NotifyConfig>>() {}.type
            gson.fromJson(json, type) ?: mutableMapOf()
        } catch (_: Throwable) { mutableMapOf() }
    }

    fun getConfig(pkg: String): NotifyConfig {
        return getAllConfigs()[pkg] ?: getGlobalConfig()
    }

    fun saveConfig(cfg: NotifyConfig) {
    ...
    /** 导出全部配置�?JSON 字符串（包含全部 SharedPreferences 键值） */
    ...
    /** �?JSON 字符串导入配置，返回是否成功（会清空当前配置后重写） */
