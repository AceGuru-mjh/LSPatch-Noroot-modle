    ...
    Column(
        ...
        Card(
            ...
            Column(
                ...
                Text("AudioBoost NoRoot", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onPrimaryContainer)
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                Text("• 音量增强（AudioTrack/MediaPlayer.setVolume）", style = MaterialTheme.typography.bodySmall)
                Text("• 低音增强（BassBoost）", style = MaterialTheme.typography.bodySmall)
                Text("• 均衡器（5段频率调节）", style = MaterialTheme.typography.bodySmall)
                Text("• [实验] 扬声器增强突破音量上限", style = MaterialTheme.typography.bodySmall)
                Text("• [实验] 麦克风增益增强", style = MaterialTheme.typography.bodySmall)
                Text("• [实验] 音质增强（提升采样率/位深）", style = MaterialTheme.typography.bodySmall)
            }
        }
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Text(
                    "仅供学习研究使用。长时间高增益可能损坏扬声器或听力，使用本模块产生的任何后果由使用者自行承担。",
