...
fun DiagnosticsScreen() {
    ...
    Column(
        ...
        DiagCard(
            ...
            detail = if (shizukuActive) "Shizuku 服务运行中（NoRoot 版仅查询用，不调用系统级音量修改�?
                     else "Shizuku 未运行或未授权。免Root版不依赖 Shizuku 即可工作",
            ok = shizukuActive
        )
        Spacer(Modifier.height(8.dp))
        ...
        Card(modifier = Modifier.fillMaxWidth(),
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Text("�?免Root版仅 Hook 应用进程内音频API，不修改系统音量服务", style = MaterialTheme.typography.bodySmall)
                Text("�?实际 Hook 是否生效需在目�?APP 内通过 LSPosed 日志验证", style = MaterialTheme.typography.bodySmall)
                Text("�?若需修改系统级音量上限，请使�?AudioBoost Pro (Root�?", style = MaterialTheme.typography.bodySmall)
            }
        }
