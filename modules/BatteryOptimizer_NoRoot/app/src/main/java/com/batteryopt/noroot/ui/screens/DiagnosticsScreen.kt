...
fun DiagnosticsScreen() {
    ...
    LaunchedEffect(Unit) {
        withContext(Dispatchers.IO) {
            try {
                ...
                data["WakeLock"] = if (cfg?.wakeLockEnabled == true) "Active (max ${cfg.wakeLockMaxHoldSec}s)" else "Off"
                data["Alarm"] = if (cfg?.alarmEnabled == true) "Active (min ${cfg.alarmMinIntervalMin}min)" else "Off"
                data["Sync"] = if (cfg?.syncEnabled == true) "Active" else "Off"
                data["Job"] = if (cfg?.jobEnabled == true) "Active" else "Off"
                data["Location"] = if (cfg?.locationEnabled == true) "Active" else "Off"
                data["Sensor"] = if (cfg?.sensorEnabled == true) "Active" else "Off"
                data["Animation"] = if (cfg?.animationEnabled == true) "Active" else "Off"
                ...
                data["Experimental"] = "$expCount active (BT/Camera/Vibrator)"
                data["AppOps"] = if (cfg?.appOpsRestrictEnabled == true) "Active (${cfg.appOpsRestrictedPackages.size} apps)" else "Off"
    ...
    Column(
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(16.dp)) {
                Text("Power Optimizations", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
                Spacer(Modifier.height(8.dp))
                DiagRow("WakeLock Control", diagData["WakeLock"] ?: "-")
                DiagRow("Alarm Optimizer", diagData["Alarm"] ?: "-")
                DiagRow("Backgnd Sync", diagData["Sync"] ?: "-")
                DiagRow("Job Scheduler", diagData["Job"] ?: "-")
