...
object ConfigManager {
    ...
    const val PREFS_NAME = "step_modifier_prefs"
    ...
    fun getGlobalConfig(): StepConfig {
        val def = StepConfig(packageName = "global")
        if (!isInitialized()) return def
        val json = prefs?.getString(KEY_GLOBAL, null) ?: return def
        return try { gson.fromJson(json, StepConfig::class.java) ?: def } catch (_: Throwable) { def }
    }

    fun saveGlobalConfig(cfg: StepConfig) {
