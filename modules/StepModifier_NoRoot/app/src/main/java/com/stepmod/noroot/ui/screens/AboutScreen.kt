    ...
    Column(
        ...
        Card(
            ...
            Column(
                ...
                Text("StepModifier NoRoot", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onPrimaryContainer)
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                Text("• 步数传感器 Hook（SensorManager/onSensorChanged）", style = MaterialTheme.typography.bodySmall)
                Text("• 步数上报 Hook（支付宝/微信/小米/华为/Keep/咕咚/悦跑圈）", style = MaterialTheme.typography.bodySmall)
                Text("• StepCounter / StepDetector 类 Hook", style = MaterialTheme.typography.bodySmall)
                Text("• [实验] 传感器阻断 / 多APP同步 / 历史伪造", style = MaterialTheme.typography.bodySmall)
            }
        }
