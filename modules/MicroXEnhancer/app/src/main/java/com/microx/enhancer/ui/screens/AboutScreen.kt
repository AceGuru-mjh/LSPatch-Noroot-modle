    ...
    Column(
        ...
        Card(
            ...
            Column(
                ...
                Text("MicroX Enhancer", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onPrimaryContainer)
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                Text("• 通用：广告净化/防撤回/界面美化/隐私增强/自动回复", style = MaterialTheme.typography.bodySmall)
                Text("• 微信专属：朋友圈保护/批量管理", style = MaterialTheme.typography.bodySmall)
                Text("• [实验] 语音导出/搜索增强/自定义主题", style = MaterialTheme.typography.bodySmall)
                Text("• 适配：绕过 Xposed 检测", style = MaterialTheme.typography.bodySmall)
            }
        }
