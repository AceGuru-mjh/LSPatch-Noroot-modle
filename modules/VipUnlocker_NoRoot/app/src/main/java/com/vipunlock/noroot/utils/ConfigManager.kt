...
object ConfigManager {
    ...
    const val PREFS_NAME = "vip_unlocker_prefs"
    ...
    fun getGlobalConfig(): VipConfig {
        val def = VipConfig(packageName = "global")
        if (!isInitialized()) return def
        val json = prefs?.getString(KEY_GLOBAL, null) ?: return def
        return try { gson.fromJson(json, VipConfig::class.java) ?: def } catch (_: Throwable) { def }
    }

    fun saveGlobalConfig(cfg: VipConfig) {
