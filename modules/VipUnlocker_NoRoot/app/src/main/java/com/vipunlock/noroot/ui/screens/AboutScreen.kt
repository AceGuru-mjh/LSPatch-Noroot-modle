    ...
    Column(
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                Text("�?音乐类：网易�?QQ音乐/酷狗/酷我 VIP 解锁", style = MaterialTheme.typography.bodySmall)
                Text("�?视频类：爱奇�?优酷/腾讯视频/B站大会员", style = MaterialTheme.typography.bodySmall)
                Text("�?阅读类：喜马拉雅/今日头条/知乎盐�?, style = MaterialTheme.typography.bodySmall)
                Text("�?工具类：百度网盘SVIP/WPS超级会员/微信读书", style = MaterialTheme.typography.bodySmall)
                Text("�?[实验] 通用VIP尝试/去广�?绕过校验", style = MaterialTheme.typography.bodySmall)
            }
        }
