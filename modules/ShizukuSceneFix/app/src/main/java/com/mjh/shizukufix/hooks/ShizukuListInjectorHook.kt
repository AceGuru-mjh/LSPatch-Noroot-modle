...
/**
 * Path B: �?Shizuku 授权列表注入 Scene
 *
 * �?shizuku/ShizukuListInjector.java �?Kotlin�?
 *
 * 多策略注入（任何一种成功即可让 Scene 出现�?Shizuku 授权列表中）�?
 ...
 *  3. Hook Shizuku API �?(rikka.shizuku.Shizuku / moe.shizuku.api.ShizukuApi �?
 *     中的 getApplication 系列 / getPackageList / queryApps 等列表型方法
 *  4. Hook Shizuku PermissionManager 任何返回 List/Map/Set 的方�?
...
object ShizukuListInjectorHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: ShizukuFixConfig) {
        ...
        LogX.i("Path B: �?Shizuku 授权列表注入 Scene 启动")
        try { LogStore.add("injected", "注入Scene到列�?) } catch (_: Exception) { }
    ...
    // ============ 策略4: Hook PermissionManager 任何返回集合的方�?============
    ...
    private fun hookListAdapterSetData(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        // ArrayAdapter.addAll / add / clear（旧�?ListView�?
